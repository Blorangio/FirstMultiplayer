...
function drawShape(shape) {
    paint.beginPath();
    paint.moveTo(shape.points[shape.points.length-1].x - camera.position.x, shape.points[shape.points.length-1].y - camera.position.y);
    for(let i in shape.points) {
        paint.lineTo(shape.points[i].x - camera.position.x, shape.points[i].y - camera.position.y);
    }
    paint.fill();
}
